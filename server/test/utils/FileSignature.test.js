...
const FileSignature_1 = require("../../src/utils/FileSignature");
describe("isValidPdf", () => {
    it("accepts a buffer starting with the PDF header", () => {
        expect((0, FileSignature_1.isValidPdf)(Buffer.from("%PDF-1.4\n...rest of file..."))).toBe(true);
    });
    it("accepts a PDF header preceded by a small preamble", () => {
        const buffer = Buffer.concat([Buffer.alloc(10), Buffer.from("%PDF-1.7")]);
        expect((0, FileSignature_1.isValidPdf)(buffer)).toBe(true);
    });
    it("rejects a file with no PDF header", () => {
        expect((0, FileSignature_1.isValidPdf)(Buffer.from("just some random text"))).toBe(false);
    });
    it("rejects the PDF header appearing after the search window", () => {
        ...
        expect((0, FileSignature_1.isValidPdf)(buffer)).toBe(false);
    });
});
/** Builds a minimal single-entry ZIP local file header, as `isValidEpub` expects to find at offset 0. */
function buildZipEntry(fileName, content, compressionMethod = 0) {
...
describe("isValidEpub", () => {
    it("accepts a zip whose first entry is a stored 'mimetype' file with the epub content type", () => {
        ...
        expect((0, FileSignature_1.isValidEpub)(buffer)).toBe(true);
    });
    it("rejects a zip whose mimetype entry is compressed (not stored)", () => {
        ...
        expect((0, FileSignature_1.isValidEpub)(buffer)).toBe(false);
    });
    ...
    it("rejects a non-zip buffer", () => {
        expect((0, FileSignature_1.isValidEpub)(Buffer.from("not a zip file at all"))).toBe(false);
    });
    it("rejects a truncated/corrupt buffer without throwing", () => {
        expect((0, FileSignature_1.isValidEpub)(Buffer.from([0x50, 0x4b, 0x03, 0x04]))).toBe(false);
    });
});
describe("isValidMobi", () => {
    it("accepts a buffer with the BOOKMOBI identifier at offset 60", () => {
        ...
        expect((0, FileSignature_1.isValidMobi)(buffer)).toBe(true);
    });
    it("rejects a buffer too short to contain the identifier", () => {
        expect((0, FileSignature_1.isValidMobi)(Buffer.alloc(50))).toBe(false);
    });
    it("rejects a buffer with unrelated bytes at offset 60", () => {
        const buffer = Buffer.alloc(68);
        buffer.write("NOTAMOBI", 60, "ascii");
        expect((0, FileSignature_1.isValidMobi)(buffer)).toBe(false);
    });
});
//# sourceMappingURL=FileSignature.test.js.map